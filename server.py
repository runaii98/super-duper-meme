...
class CORSRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def remove_nginx_config(self, user_id, random_str):
        ...
        path = f"/{user_id}/ComfyUI/{random_str}/"
        # Simplified config_block for removal (no need for proxy_pass details)
        config_block = f"location {path} {{ }}"
        try:
            cmd = ['docker', 'exec', NGINX_CONTAINER, 'cat', '/etc/nginx/conf.d/default.conf']
            result = subprocess.run(cmd, capture_output=True, text=True)
    ...
    def do_POST(self):
        ...
        if self.path == '/register':
            ...
            docker_cmd = [
                ...
                '--gpus', '"device=0"',
                '--shm-size=8g',
                '--env', 'COMFYUI_ARGS="--fp16 --multi-gpu"',
                f'-p', f'{port_no}:8188',
                '-v', f'{SHARED_DIR}:/comfy/mnt',
                '--user', '1024:1024',
                '--name', container_name,
